...
export default function HomeHeroMain() {
	const { navigate, formatMessage } = useHomeHeroContext();
	return (
		<div
				...
				<div className="max-w-3xl">
					<div className="flex items-center mb-4">
						<h1 className="text-5xl md:text-6xl font-bold  mb-6">
							{formatMessage(Messages.HomeHero.title)}
						</h1>
					</div>
					<p className="text-lg md:text-xl  dark:text-gray-200 mb-8 leading-relaxed">
						{formatMessage(Messages.HomeHero.description)}
					</p>
					<div className="flex items-center gap-4 justify-between">
						...
						>
							{formatMessage(Messages.HomeHero.getStarted)}
						</button>
					</div>
				</div>
			</div>
